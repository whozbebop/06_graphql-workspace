...
const resolvers = {
  ...
  Mutation: {
    createPost: (_, {title, content, authorId}) => { // args === {title: xx, content: xx, authorId: xx }
    ...
    deletePost: (_, { id }) => {
      ...
      // DB(배열)로부터 해당 id를 가진 게시글 객체 인덱스 찾기
      const deletedIndex = posts.findIndex(post => post.id === id)
       
      // splice 활용해서 삭제 (삭제된 요소도 반환)
      const deletedPost = posts.splice(deletedIndex, 1)[0]
      // [삭제된요소객체]
